...
# Multi-stage optimized Dockerfile for PNL Pipeline Container
...
RUN --mount=type=cache,target=/tmp/download-cache \
    set -e; \
    RPM="freesurfer-Rocky8-${FS_VER}-1.x86_64.rpm"; \
    wget -q "https://surfer.nmr.mgh.harvard.edu/pub/dist/freesurfer/${FS_VER}/${RPM}" -O "/tmp/download-cache/${RPM}"; \
    rpm2cpio "/tmp/download-cache/${RPM}" | cpio -idmv; \
...
# Make conda base binaries visible (optional; envs are set in .bashrc)
ENV PATH="/opt/conda/bin:$PATH"
